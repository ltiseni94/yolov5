...
CLASSES = [39, 40, 41, 42, 43, 44, 46, 47, 63, 65, 67, 73, 78, 79]
...
def run(
    ...
    grasp_score = GraspScore(close_weight=0.1, proximity_weight=0.1)
    ...
    with mp_hands.Hands(
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for obj in tracked_objects:
                norfair_tracked.append(obj.filter.x.squeeze()[:4])

            prox_score, obj_idx = proximity_score(norfair_tracked, landmarks_list=landmark_list)
            grasp_prob, grasp_label = grasp_score(close_prob, prox_score)
            if len(tracked_objects) > 0 and obj_idx is not None:
                print(f'{grasp_prob:.2f} - {names[int(tracked_objects[obj_idx].last_detection.data[0])]}')
            else:
                print(f'{grasp_prob:.2f}')
            dt[2] += time_sync() - t3
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if view_img:
                    ...
                    if grasp_prob > grasp_score.conf_thres:
                        cv2.putText(im0, f'GRASP: {grasp_prob:.2f}', (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 3)
                        cv2.putText(im0, f'GRASP: {grasp_prob:.2f}', (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
                    else:
                        cv2.putText(im0, f'GRASP: {grasp_prob:.2f}', (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)

                    norfair.draw_points(im0, norfair_dets, 5, 2, (0, 255, 0))
                    norfair.draw_tracked_objects(im0, tracked_objects, 5, (0, 255, 0), 3, 2)
